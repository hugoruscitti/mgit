...
def obtener_progreso(cantidad, total):
	progress = cantidad / total
	ancho = 15
	cantidad_de_caracteres_para_el_progreso = math.floor(ancho * progress)

	barra_de_progreso = ("▓" * cantidad_de_caracteres_para_el_progreso)
	barra_restante = ("░" * (ancho -cantidad_de_caracteres_para_el_progreso))

	return  f"{verde}|{barra_de_progreso}{gris}{barra_restante}{verde}|{fin}"
